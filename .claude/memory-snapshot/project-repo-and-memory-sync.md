...
**How to apply:** `git pull` at the start of a session, `git add -A && git commit -m "..." && git push` before it ends — nothing reaches GitHub on its own. On a new machine set `autoMemoryDirectory` to that machine's own absolute path. Excluded from the repo by design: `raw/`, `outputs/`, `node_modules/`, `.env` — and since 2026-09-02 also `graphics-build/out/` and `graphics-build/work/` (rendered PRODUCTS). See [[feedback-lessons-into-skills]].

**Renders never enter git (2026-09-02, standard-deviation job).** GitHub's pre-receive hook hard-rejects any blob over 100MB and there is no LFS on this repo; ProRes beat renders routinely exceed that (base-plate 301MB, captions.mov 326MB). A commit that includes one is unpushable and has to be rewritten — which is exactly what happened: the previous session's commit sat local-only overnight because its push died on size, and the "pushed" state in the session summary was wrong. Everything in `out/` and `work/` regenerates from tracked source, so the fix is policy, not LFS. Watch for it anyway: a `git status` showing a modified 100MB+ `.mov` under a path NOT covered by the ignore rules means the rules drifted.

**The Orca pre-commit hook needs the user's login env (2026-09-02).** A JAMF-managed global hook (`~/.git-templates/hooks/pre-commit`, `core.hooksPath`) runs `orca-cli secrets pre-commit scan` and aborts without `ORCA_SECURITY_PROJECT_KEY`. The key lives in `~/.zprofile`/`~/.zshrc`, which non-interactive shells do not source — so Claude's commits fail where the user's terminal commits succeed. Run `source ~/.zprofile` in the same command as `git commit`. Never `--no-verify` around it: it is a corporate secret-scanning gate, and the scan passing is the point.
