...
> **Vendored into this repo 2026-08-17** so the mandatory review loop works on every machine
> (CLAUDE.md: "The review loop is not optional"). Per-machine setup still needed once:
> `python3 .claude/skills/watch/scripts/setup.py` installs ffmpeg/yt-dlp and scaffolds
> `~/.config/watch/.env` for the Whisper API key (keys never live in the repo).
